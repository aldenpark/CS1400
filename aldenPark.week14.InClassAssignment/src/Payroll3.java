...
public class Payroll3 {
    public static void main(String[] args)
    {
        ...
        ((Commission) employees[3]).addSales(1500);
        ((Commission) employees[4]).addSales(1500);
        
        // This arbitrarily assumes tha the payroll's month starts on a 
        ...
        for(int date=1,day=2; date<=30;date++,day++,day++,day%=7) {
            for(int i=0; i<employees.length && employees[i] != null; i++) {
                if(day > 0 && day < 6 && employees[i] instanceof Hourly2) {
                    hourly = (Hourly2) employees[i];
                    hourly.addHours(8);
                }
                if((day == 5 && employees[i] instanceof Hourly2) || 
                    (date%15 == 0 && (employees[i] instanceof Salaried2 ||
